...
- [x] Load actual `.mem` files into RTL
- [x] Feed actual quantized inputs into RTL
- [x] Use actual trained weights
- [x] Use actual quantized biases
- [x] Execute the trained model inside RTL
- [x] Capture the RTL prediction
- [x] Compare RTL output with Python quantized inference
- [x] Verify predicted classes match
- [x] Automate Python-vs-RTL verification

### FPGA Implementation & Control Architecture

- [x] Add hardware memory/ROM interfaces (`rom_sync.sv`)
- [x] Add `$readmemh()` on-chip model loading
- [x] Add start/done/ready control interface
- [x] Add control FSM (`tinynpu_fpga.sv`)
- [x] Add multi-cycle layer sequencing & pipeline registers
- [x] FPGA synthesis constraints (`tinynpu.xdc`)
- [x] Automated Vivado synthesis flow (`synth_vivado.tcl`)
- [x] Open-source Yosys synthesis flow (`synth_yosys.ys`)
- [x] Measure LUT, FF, DSP, and BRAM utilization
- [x] Measure clock frequency ($F_{\text{max}} = 127.4\text{ MHz}$)
- [x] Measure latency (3 clock cycles per inference = 30 ns @ 100MHz)
- [x] Measure throughput (25 to 100 MSamples/sec)
- [x] Measure power consumption (< 85 mW)

### Hardware Optimization

- [x] Pipelined streaming accelerator (`tinynpu_pipelined.sv`, $II=1$)
- [x] Resource-shared 1-MAC engine (`tinynpu_resource_shared.sv`)
- [x] 99.7% Multiplier area reduction (399 to 1 multiplier)
- [x] Multi-architecture simulation verification (`tb_tinynpu_resource_shared.sv`)
- [x] Comprehensive synthesis and optimization report (`docs/FPGA_Synthesis_and_Optimization_Report.md`)
...
Convert floating-point model to INT8/INT32 representation.
**Status: COMPLETE**

---

### Phase 3 — Quantized Inference
Run inference using quantized representation and validate reference outputs.
**Status: COMPLETE**

---

### Phase 4 — Hardware Data Export
Export quantized parameters to `.mem` hex files and `tinynpu_params.vh`.
**Status: COMPLETE**

---

### Phase 5 — RTL Datapath
Implement fundamental SystemVerilog hardware modules (`mac.sv`, `relu.sv`, `requantize.sv`, `neuron.sv`).
**Status: COMPLETE**

---

### Phase 6 — RTL Unit Verification
Verify all individual components, layer modules, and top-level network with testbenches.
**Status: COMPLETE**

---

### Phase 7 — Real Model RTL Verification
Load actual `.mem` weights into RTL and verify 100% bit-accurate parity against Python.
**Status: COMPLETE**

---

### Phase 8 — FPGA Sequential Control & Memory Architecture
Implement clocked sequential FSM (`tinynpu_fpga.sv`), on-chip ROMs, timing constraints (`.xdc`), and Vivado/Yosys synthesis scripts.
**Status: COMPLETE**

---

### Phase 9 — Hardware Optimization & Benchmarking
Implement pipelined streaming ($II=1$) and ultra-low-area resource-shared 1-MAC engine with area/timing benchmarks.
**Status: COMPLETE**
